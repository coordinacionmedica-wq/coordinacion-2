...
export default function App() {
  ...
  const [activeTab, setActiveTab] = useState<'home' | 'turnos' | 'solicitudes' | 'novedades' | 'rural' | 'bd' | 'docs' | 'admin' | 'ayuda' | 'stats' | 'pic' | 'toolbox'>('turnos');
    ...
    const unsubAuth = onAuthStateChanged(auth, async (user) => {
      ...
      if (user) {
        ...
        if (isWhitelisted) {
          ...
          // Try to sync to DB but don't block login if it fails
          ...
          // Check if admin doc exists for this user via query
          try {
            ...
            if (!adminSnap.empty) {
              ...
              setNotification({ 
                message: `Google Login: ${user.email} conectado, pero no tiene permisos de administrador.`, 
                type: 'info' 
              });
            }
          } catch (err) {
        ...
        // If no user exists at all (not even anonymous), sign in anonymously
        // But DON'T clear manual session if it exists and we're just waiting for auth
...
Donde doctorId es el ID numérico del médico y las llaves de los días son del 1 al ${daysInMonth}.`;
  ...
  const downloadTemplateExcel = () => {
    ...
    doctors.filter(d => d.st === 'activo').sort((a, b) => a.nombre.localeCompare(b.nombre)).forEach(med => {
      (['m', 't', 'n'] as SlotType[]).forEach((slot) => {
        const rowData: any[] = [
          med.id, 
          med.nombre, 
