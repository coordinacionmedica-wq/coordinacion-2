        ...
        <div className="bg-white p-8 rounded-[32px] border border-slate-200 shadow-xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <select className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl outline-none text-slate-800" value={reqDay} onChange={e => setReqDay(parseInt(e.target.value))}>
            ...
            <div>
              ...
              <select className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl outline-none text-slate-800" value={reqSlot} onChange={e => setReqSlot(e.target.value as SlotType)}>
            ...
            <div className="sm:col-span-3">
              ...
              <textarea className="w-full bg-slate-50 border border-slate-200 p-4 rounded-xl outline-none focus:border-sky-500 min-h-[100px] text-slate-800" placeholder="Describa el cambio solicitado..." value={reqReason} onChange={e => setReqReason(e.target.value)} />
            </div>
          </div>
          <button onClick={onSubmit} className="w-full mt-4 bg-sky-500 text-white font-black h-16 rounded-2xl hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3 shadow-lg shadow-sky-500/20">
              ...
              <div key={req.id} className="bg-stone-50 p-6 rounded-2xl border border-emerald-100 flex flex-wrap items-center justify-between gap-4 shadow-sm hover:border-emerald-500/30 transition-colors">
                ...
                {isAdmin && req.status === 'pending' && (
                  <div className="flex gap-2">
                    <button onClick={() => onUpdateStatus(req.id, 'approved')} className="bg-emerald-500/10 hover:bg-emerald-500 text-emerald-500 hover:text-white p-3 rounded-xl border border-emerald-500/30 transition-all">
                      <CheckCircle className="w-5 h-5" />
                    </button>
                    <button onClick={() => onUpdateStatus(req.id, 'rejected')} className="bg-rose-500/10 hover:bg-rose-500 text-rose-500 hover:text-white p-3 rounded-xl border border-rose-500/30 transition-all">
