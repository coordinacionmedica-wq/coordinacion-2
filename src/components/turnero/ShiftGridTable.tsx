...
export function ShiftGridTable(props: ShiftGridTableProps) {
  ...
  // Mobile card view for a single doctor
  const MobileDoctorCard = ({ med }: { med: Doctor }) => {
    let medTotalMonth = 0;
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dayHours = (variables.m[m] || 0) + (variables.t[t] || 0) + (variables.n[n] || 0);
      medTotalMonth += dayHours;

      const wIdx = sundays.findIndex(sunD => d <= sunD);
      if (wIdx !== -1) weeklyAcc[wIdx] += dayHours;

      daysData.push({ day: d, dow, isWeekend, isSunday, m, t, n, hours: dayHours });
    }
        ...
        <div className="p-3">
          <div className="grid grid-cols-7 gap-1">
            ...
            {daysData.map(({ day, dow, isWeekend, isSunday, m, t, n }) => {
              ...
              const hasPT = m === 'PT' || t === 'PT' || n === 'PT';
              const shiftCount = [m, t, n].filter(v => v !== 'X' && v !== 'PT').length;

              return (
                <button
                  key={day}
                  onClick={() => {
                    // Cycle through slots to edit on mobile tap
                    if (!isAdmin) return;
                    const slots: SlotType[] = ['m', 't', 'n'];
                    const currentSlot = editingCell?.doctorId === med.id && editingCell?.day === day
                      ? slots[(slots.indexOf(editingCell.slot) + 1) % 3]
                      : 'm';
                    const val = currentMonthData[med.id]?.[currentSlot]?.[day] || 'X';
                    setEditingCell({ doctorId: med.id, day, slot: currentSlot });
                    ...
                    <div className="absolute inset-0 bg-emerald-50 rounded-lg flex items-center justify-center">
                      <input
                        ...
                        onKeyDown={e => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            handleSetShift(med.id, day, editingCell.slot, editingValue || 'X');
                          }
                          if (e.key === 'Escape') setEditingCell(null);
                        }}
                        onBlur={() => handleSetShift(med.id, day, editingCell.slot, editingValue || 'X')}
                        className="w-full text-center bg-transparent outline-none text-emerald-700 font-black text-xs"
                        maxLength={4}
                      />
                    </div>
                  )}
  ...
  return (
    <div className="relative" onPaste={handleBulkPaste}>
      ...
      <div
        ...
        className="hidden md:block overflow-auto border border-slate-200 rounded-xl md:rounded-[18px] bg-white shadow-xl max-h-[calc(100vh-280px)] custom-scrollbar -mx-1 md:mx-0 select-none"
      >
